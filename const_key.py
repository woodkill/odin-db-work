"""
 collection key
"""
kCOL_ODINDATA = "OdinData"
kCOL_ODINGUILD = u'OdinGuild'
kCOL_BOSS = u'Boss'

"""
 document key
"""
kDOC_ODIN_SERVER = u'OdinServer'
kDOC_ODIN_BOSS = u'OdinBoss'
kDOC_ODIN_CHAPTER_BOSS = u'OdinChapterBoss'

"""
 field name
"""
kFLD_CHAP_BOSS_LIST = u'chapBossList'
kFLD_SERVER_LIST = u'serverList'
kFLD_BOSS_DIC = u'bossDictionary'
kFLD_SERVER_NAME = u'serverName'
kFLD_GUILD_NAME = u'guildName'
kFLD_CHANNEL_ID = u'channelId'
kFLD_ALARMS = u"alarms"

"""
 dictionary key
"""
kSERVER_NAME = u'serverName'
kCHAP_ORDER = u'chapOrder'
kCHAP_NO = u'chapNumber'
kCHAP_NAME = u'chapName'
kCHAP_ALIAS = u'chapAlias'
kCHAP_COLOR_CODE = u'chapColorBlock'
kCLICK_POS_S = u'clickPosSmall'  # 해상도 1920 x .. 용
kCLICK_POS = u'clickPos'  # 해상도 2560 x .. 용
kBOSS_THUMBNAIL_URL = u'bossThumbnailUrl'
kBOSS_LEVEL = u'bossLevel'
kBOSS_ORDER = u'bossOrder'
kBOSS_NAME = u'bossName'
kBOSS_ALIAS = u'bossAlias'
kBOSS_TYPE = u'bossType'
kBOSS_FIXED_TIME = u'bossFiexedTime'
kBOSS_INTERVAL = u'bossInterval'
kBOSS_WEEKDAY_INFO = u'bossWeekdayInfo'
kBOSS_WEEKDAY = u'bossWeekday'
kBOSS_APPEARANCE_TIME = u'bossApprearanceTime'
kBOSS_LIST = u'bossList'
kBOSS_SUPREME = u'bossSupreme'  # 보스가 절대자인지 아닌지

kX = 'x'
kY = 'y'
kW = 'width'
kH = 'height'


